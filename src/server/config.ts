...
export const mosaicConfigSchema = z.object({
  outputWidth: z.number().int().min(256).max(12000).default(4800),
  outputHeight: z.number().int().min(256).max(12000).default(3200),
  targetMegapixels: z.number().min(0).max(80).default(0),
  tileSize: z.number().int().min(8).max(512).default(40),
  ...
  mainImageOpacity: z.number().min(0).max(1).default(0.18),
  colorMatchingStrength: z.number().min(0).max(1).default(0.6),
  repeatLimit: z.number().int().min(1).max(9999).default(3),
  minRepeatSpacing: z.number().int().min(0).max(10000).default(18),
  candidatePoolLimit: z.number().int().min(10).max(20000).default(500),
  usePreviews: z.boolean().default(true),
