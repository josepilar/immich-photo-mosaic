...
describe('output generation', () => {
  ...
  it('moves tile hue toward target cell colors', async () => {
    ...
    const main = await sharp({ create: { width: 48, height: 48, channels: 3, background: '#ff0000' } }).png().toBuffer()
    const blue = await sharp({ create: { width: 48, height: 48, channels: 3, background: '#0000ff' } }).png().toBuffer()
    const candidates: Array<TileCandidate> = [{ assetId: 'blue', buffer: blue, average: await imageAverage(blue) }]
    const config = { ...defaultConfig.mosaic, outputWidth: 48, outputHeight: 48, tileSize: 48, repeatLimit: 99, outputFormat: 'png' as const, colorMatchingStrength: 1, mainImageOpacity: 0 }
    const result = await renderMosaic({ mainBuffer: main, candidates, config, outputFolder: dir })
  ...
  it('detects screenshot-like candidates without rejecting photo-like gradients', async () => {
    const screenshot = await sharp(Buffer.from('<svg width="240" height="160"><rect width="240" height="160" fill="white"/><rect x="12" y="12" width="216" height="20" fill="#111"/><rect x="12" y="48" width="180" height="12" fill="#333"/><rect x="12" y="76" width="205" height="12" fill="#333"/><rect x="12" y="104" width="120" height="12" fill="#333"/></svg>')).png().toBuffer()
    const photoLike = await sharp({ create: { width: 240, height: 160, channels: 3, background: '#7aa06f' } })
      .composite([{ input: await sharp({ create: { width: 120, height: 160, channels: 3, background: '#d39b75' } }).blur(20).png().toBuffer(), left: 60, top: 0 }])
      .png()
      .toBuffer()
